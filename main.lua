...
function love.load()
	...
	player.speed = 500    -- This is the player's speed. This value can be change based on your liking.
	...
	--unless you want the game's physics to not work the usual way, player.gravity > player.jump_height
...
function love.update(dt)
	...
	if love.keyboard.isDown('d') then                    -- When the player presses and holds down the "D" button:
		...
		end   -- The player moves to the right.
	...
	elseif love.keyboard.isDown('a') then                -- When the player presses and holds down the "A" button:
		...
		if player.x > 0 then 
			player.x = player.x - (player.speed * dt)
		end   -- The player moves to the left.
	end
